...
import { Store } from ".";
import { Asset, Pool, Network } from "@sifchain/sdk";
...
export const createPoolFinder: PoolFinderFn =
  (s: Store) => (a: Asset | string, b: Asset | string) => {
...
export const createAccountPoolFinder: AccountPoolFinderFn =
  (s: Store) => (a: Asset | string, b: Asset | string) => {
    if (typeof a === "string") a = Asset.get(a);
    if (typeof b === "string") b = Asset.get(b);
