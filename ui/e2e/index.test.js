...
describe.only("Import/export", () => {
  it.only("imports rowan", async () => {
    ...
    // assert.equal(page.url(), 'http://localhost:8080/#/balances');

    // Click text=ETHImport >> a
    await page.click("text=ETHImport >> a");
    // assert.equal(page.url(), 'http://localhost:8080/#/balances/import/ceth/select?network=ethereum&amount=');

    // Click text=Tokeneth >> button
    await page.click("text=Tokeneth >> button");

    // Click text=Tokeneth >> button
    await page.click("text=Tokeneth >> button");

    // Click text=Connect Sifchain Wallet
    await page.click("text=Connect Sifchain Wallet");

    // Click text=irisKeplrConnect
    await page.click("text=irisKeplrConnect");

    // Open new page
    await context.waitForEvent("page");
    const page12 = await getExtensionPage(
      "dmkamcknogkgcdfhhbddcghachkejeap",
      "/popup.html#/suggest-chain?interaction=true&interactionInternal=false",
    );

    // Click text=Approve
    await page12.click("text=Approve");
    ...
    // Open new page
    await context.waitForEvent("page");
    const page13 = await getExtensionPage(
      "dmkamcknogkgcdfhhbddcghachkejeap",
      "/popup.html#/access?interaction=true&interactionInternal=false",
    );

    // Click text=Approve
    await page13.click("text=Approve");

    // Close page
    await page13.close();

    // Click text=Import Token to SifchainNetworkEthereumTokenethBalance: 0MAXSifchain Recipient A >> button
    await page.click(
      "text=Import Token to SifchainNetworkEthereumTokenethBalance: 0MAXSifchain Recipient A >> button",
    );
    // assert.equal(page.url(), 'http://localhost:8080/#/balances');

    // Click [placeholder="Search Token..."]
    await page.click('[placeholder="Search Token..."]');

    // Fill [placeholder="Search Token..."]
    await Promise.all([
      page.waitForNavigation(/*{ url: 'http://localhost:8080/#/balances?q=eth&focused=' }*/),
      page.fill('[placeholder="Search Token..."]', "eth"),
    ]);
