...
exports.calculatePriceImpact =
  exports.calculateProviderFee =
  exports.calculateReverseSwapResult =
  exports.calculateExternalExternalSwapResult =
  exports.calculateSwapResult =
  exports.calculateWithdrawal =
  exports.calculatePoolUnits =
  exports.slipAdjustment =
    void 0;
const Amount_1 = require("./Amount");
function slipAdjustment(
  ...
  return (0, Amount_1.Amount)("1").subtract(slipAdjustmentReciprocal);
}
...
const TEN_THOUSAND = (0, Amount_1.Amount)("10000");
function calculateWithdrawal({
  ...
  let unitsToClaim = (0, Amount_1.Amount)("0");
  if (!wBasisPoints.equalTo("0")) {
    unitsToClaim = lpUnits.divide(TEN_THOUSAND.divide(wBasisPoints));
  }
  let poolUnitsOverUnitsToClaim = (0, Amount_1.Amount)("0");
  if (!unitsToClaim.equalTo("0")) {
    poolUnitsOverUnitsToClaim = poolUnits.divide(unitsToClaim);
  }
  let withdrawExternalAssetAmountPreSwap = (0, Amount_1.Amount)("0");
  let withdrawNativeAssetAmountPreSwap = (0, Amount_1.Amount)("0");
...
function calculateReverseSwapResult(S, X, Y) {
  ...
  if (
    S.equalTo("0") ||
    X.equalTo("0") ||
    S.multiply((0, Amount_1.Amount)("4")).greaterThan(Y)
  ) {
    return (0, Amount_1.Amount)("0");
  }
  const term1 = (0, Amount_1.Amount)("-2").multiply(X).multiply(S);
  const term2 = X.multiply(Y);
  const underRoot = Y.multiply(
    Y.subtract(S.multiply((0, Amount_1.Amount)("4"))),
  );
  const term3 = X.multiply(underRoot.sqrt());
  const numerator = term1.add(term2).subtract(term3);
  const denominator = S.multiply((0, Amount_1.Amount)("2"));
  const x = numerator.divide(denominator);
  return x.greaterThanOrEqual((0, Amount_1.Amount)("0"))
    ? x
    : (0, Amount_1.Amount)("0");
}
