  ...
  decode(input, length) {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3:
          message.claims.push(
            types_2.EthBridgeClaim.decode(reader, reader.uint32()),
          );
          break;
        default:
          reader.skipType(tag & 7);
  ...
  toJSON(message) {
    ...
    message.status !== undefined &&
      (obj.status = message.status
        ? types_1.Status.toJSON(message.status)
        : undefined);
    if (message.claims) {
      obj.claims = message.claims.map((e) =>
        e ? types_2.EthBridgeClaim.toJSON(e) : undefined,
      );
    } else {
      obj.claims = [];
    }
...
class QueryClientImpl {
  ...
  EthProphecy(request) {
    ...
    return promise.then((data) =>
      exports.QueryEthProphecyResponse.decode(
