...
let setup = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const wallet = new DirectSecp256k1HdWalletProvider_1.DirectSecp256k1HdWalletProvider(config, {
    ...
    console.log((yield wallet.fetchBalances(chains.native, nativeAddress)).map((s) => s.toString()));
    console.log((yield wallet.fetchBalances(chains.counterparty, counterpartyAddress)).map((s) => s.toString()));
...
describe("IBCBridge", () => {
    ...
    test("it transfers into sifchain", () => __awaiter(void 0, void 0, void 0, function* () {
        const { wallet, chains, bridge, nativeAddress, counterpartyAddress } = yield setupPromise;
        const counterpartyAsset = chains.counterparty.lookupAssetOrThrow(chains.counterparty.chainConfig.nativeAssetSymbol);
        const decimalAmount = (0, utils_1.toBaseUnits)("0.001", counterpartyAsset);
        const importParams = {
            fromChain: chains.counterparty,
            toChain: chains.native,
            assetAmount: (0, entities_1.AssetAmount)(counterpartyAsset, decimalAmount),
            fromAddress: counterpartyAddress,
            toAddress: nativeAddress,
        };
        const nativeBalanceOfCounterAsset = yield wallet.fetchBalance(chains.native, nativeAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        expect(+nativeBalanceOfCounterAsset.amount.toBigInt().toString()).toEqual(0);
        const bridgeTx = yield bridge.transfer(wallet, importParams);
        expect(bridgeTx.assetAmount.toString()).toEqual(importParams.assetAmount.toString());
        ...
        const newCounterpartyNativeBalance = yield wallet.fetchBalance(chains.native, nativeAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        expect(newCounterpartyNativeBalance.toBigInt().toString()).toEqual(decimalAmount);
    }));
    test("it transfers out of sifchain", () => __awaiter(void 0, void 0, void 0, function* () {
        const { wallet, chains, bridge, nativeAddress, counterpartyAddress } = yield setupPromise;
        const nativeBalanceOfCounterAsset = yield wallet.fetchBalance(chains.native, nativeAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        ...
        const counterpartyBalance = yield wallet.fetchBalance(chains.counterparty, counterpartyAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        const bridgeTx = yield bridge.transfer(wallet, exportParams);
        expect(bridgeTx.assetAmount.toString()).toEqual(exportParams.assetAmount.toString());
        expect(bridgeTx.fromChain).toBe(exportParams.fromChain);
        expect(bridgeTx.toChain).toBe(exportParams.toChain);
        const result = yield bridge.waitForTransferComplete(wallet, bridgeTx);
        expect(result).toEqual(true);
        const newNativeCounterpartyBalance = yield wallet.fetchBalance(chains.native, nativeAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        const newCounterpartyBalance = yield wallet.fetchBalance(chains.counterparty, counterpartyAddress, chains.counterparty.chainConfig.nativeAssetSymbol);
        expect(newNativeCounterpartyBalance.toBigInt().toString().charAt(0)).toEqual("0");
        expect(newCounterpartyBalance.toString()).toEqual(counterpartyBalance.add(decimalAmount).toString());
    }));
});
