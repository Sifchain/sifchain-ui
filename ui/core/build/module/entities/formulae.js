...
export function slipAdjustment(r, // Native amount added
 ...
 * @returns
 */
export function calculatePoolUnits(r, // Native amount added
a, // External amount added
R, // Native Balance (before)
...
export function calculateWithdrawal({ poolUnits, nativeAssetBalance, externalAssetBalance, lpUnits, wBasisPoints, asymmetry, }) {
 ...
 * @returns swapAmount
 */
export function calculateSwapResult(x, X, Y) {
...
export function calculateReverseSwapResult(S, X, Y) {
 ...
 * @returns providerFee
 */
export function calculateProviderFee(x, X, Y) {
    if (x.equalTo("0") || X.equalTo("0") || Y.equalTo("0")) {
        return Amount("0");
    }
    const xPlusX = x.add(X);
 ...
 * @returns
 */
export function calculatePriceImpact(x, X) {
    if (x.equalTo("0")) {
        return Amount("0");
    }
